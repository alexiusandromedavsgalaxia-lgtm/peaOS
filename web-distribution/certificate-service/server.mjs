...
if(!GITHUB_TOKEN)console.warn('peaOS certificate service: PEAOS_GITHUB_TOKEN is not configured; GitHub publication is disabled.');
...
function validCertificate(c){return c&&c.version===CERTIFICATE_VERSION&&c.type==='SigningCertificate'&&/^[a-f0-9]{32}$/.test(c.certificate_id)&&/^[a-f0-9]{32}$/.test(c.issuer_key_id)&&/^[a-f0-9]{64}$/.test(c.serial)&&Number.isSafeInteger(c.issued_at)&&Number.isSafeInteger(c.expires_at)&&c.expires_at>c.issued_at&&c.expires_at-c.issued_at<=365*24*60*60&&typeof c.public_key==='string'&&typeof c.signature==='string'&&typeof c.sha512==='string'&&c.sha512===certificateSha512(c);}
...
async function publishCertificate(c,input){
 const certPath=`${CERTIFICATE_DIR}/${c.certificate_id}.json`;
 for(let attempt=0;attempt<3;attempt++){
  const {registry,sha}=await readRegistry();
  if(registry.certificates.some(x=>x.id===c.certificate_id))return;
  registry.certificates.push({id:c.certificate_id,type:c.type,status:'Active',issuer_key_id:c.issuer_key_id,public_key:c.public_key,serial:c.serial,issued_at:c.issued_at,expires_at:c.expires_at,sha512:c.sha512});
  try{await writeFile(REGISTRY_PATH,`${JSON.stringify(registry,null,2)}\n`,sha,`Register signing certificate ${c.certificate_id}`);break}catch(error){if(!String(error.message).includes('does not match')||attempt===2)throw error;}
 }
 await writeFile(certPath,certificatePayload(c),null,`Publish signing certificate ${c.certificate_id}`);
}
function issue(){const now=Math.floor(Date.now()/1000);const {publicKey:subjectPrivate,privateKey:subjectKey}=crypto.generateKeyPairSync('ed25519');const subjectPublic=subjectPrivate;const c={version:CERTIFICATE_VERSION,type:'SigningCertificate',certificate_id:crypto.randomBytes(16).toString('hex'),issuer_key_id:issuerKeyId,issued_at:now,expires_at:now+365*24*60*60,serial:nextSerial(),public_key:subjectPublic.export({type:'spki',format:'der'}).toString('base64')};c.signature=sign(c);c.sha512=certificateSha512(c);return{certificate:c,private_key:subjectKey.export({type:'pkcs8',format:'pem'})};}
const server=http.createServer(async(req,res)=>{
 if(req.method==='OPTIONS')return json(res,204,{});
 const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);
 if(req.method==='GET'&&url.pathname==='/v1/issuer')return json(res,200,{protocol:3,issuer:REGISTRY_ISSUER,issuer_key_id:issuerKeyId,algorithm:'Ed25519',certificate_version:CERTIFICATE_VERSION,public_key:publicKey.export({type:'spki',format:'der'}).toString('base64')});
 const match=url.pathname.match(/^\/v1\/certificates\/([a-f0-9]{32})$/);
 if(req.method==='GET'&&match){const r=state.get(match[1]);if(!r)return json(res,404,{active:false,reason:'unknown_certificate'});return json(res,200,{certificate:r.certificate,active:active(r),revoked:r.revoked,reason:r.revoked?'revoked':active(r)?'active':'expired'});}
 if(req.method==='POST'&&(url.pathname==='/v1/certificates/issue'||url.pathname==='/v1/registrations')){
  if(!authorized(req))return json(res,401,{ok:false,reason:'unauthorized'});
  let input;try{input=await body(req);}catch{return json(res,400,{ok:false,reason:'invalid_json'});}
  if(!validName(input.name))return json(res,400,{ok:false,reason:'invalid_name'});
  if(!GITHUB_TOKEN)return json(res,503,{ok:false,reason:'github_backend_not_configured'});
  try{const issued=issue(),r={certificate:issued.certificate,revoked:false,owner:String(input.name).trim()};if(!validCertificate(issued.certificate))return json(res,500,{ok:false,reason:'certificate_self_check_failed'});await publishCertificate(issued.certificate,input);state.set(issued.certificate.certificate_id,r);persist();return json(res,201,{ok:true,certificate_id:issued.certificate.certificate_id,type:'SigningCertificate',certificate:issued.certificate,private_key:issued.private_key});}catch(error){return json(res,502,{ok:false,reason:error.message});}
 }
 const revoke=url.pathname.match(/^\/v1\/certificates\/([a-f0-9]{32})\/revoke$/);
 if(req.method==='POST'&&revoke){if(!authorized(req))return json(res,401,{active:false,reason:'unauthorized'});const r=state.get(revoke[1]);if(!r)return json(res,404,{active:false,reason:'unknown_certificate'});r.revoked=true;persist();return json(res,200,{active:false,reason:'revoked'});}
